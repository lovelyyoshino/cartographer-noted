...
std::unique_ptr<transform::Rigid2d> LocalTrajectoryBuilder2D::ScanMatch(
  ...
  //okagv
  //common::Duration duration_match = time - last_match_time_;
  // || common::ToSeconds(duration_match) > 1.0
  ...
  if (options_.use_intensity_ceres_scan_matching()){
    ceres_scan_matcher_.MatchWithIntensity(
        pose_prediction.translation(), initial_ceres_pose,
        filtered_gravity_aligned_point_cloud, *matching_submap->grid(),
        *matching_submap->intensity_grid(), pose_observation.get(), &summary);
  } else {
    // old method
  ...
  if (pose_observation) {
    ...
    //LOG(INFO) << "okagv residual_distance " <<  residual_distance 
...
LocalTrajectoryBuilder2D::AddRangeData(
  ...
  if (num_accumulated_ == 0) {
    ...
    // accumulation.
    accumulated_range_data_ = sensor::RangeData{{}, {}, {}}; //okagv ,add intensity
  }
  ...
  for (size_t i = 0; i < synchronized_data.ranges.size(); ++i) {
    ...
    if (range >= options_.min_range()) {
      if (range <= options_.max_range()) {
        hit_in_local.intensity = synchronized_data.ranges[i].intensity; //okagv
  ...
  //okagv
  //LOG(INFO) << "okagv check accumulated_range_data_ size" << accumulated_range_data_.returns.size();
  //if(accumulated_range_data_.returns.size() < )
  ++num_accumulated_;
...
void LocalTrajectoryBuilder2D::RegisterMetrics(
  ...
  //okagv
  void LocalTrajectoryBuilder2D::ClearOldData()
  {
     active_submaps_.clearActiveSubmaps();
  }
