...
MapBuilder::MapBuilder(proto::MapBuilderOptions& options)
    : options_(options), thread_pool_(options.num_background_threads()) {
  CHECK(options.use_trajectory_builder_2d() ^
        options.use_trajectory_builder_3d());
  if (options.use_trajectory_builder_2d()) {
    pose_graph_ = absl::make_unique<PoseGraph2D>(
        const_cast<::cartographer::mapping::proto::PoseGraphOptions&>(
            options_.pose_graph_options()),  // okagv const to non-const
  ...
  if (options.collate_by_trajectory()) {
    LOG(INFO) << "okagv using TrajectoryCollator" ;
    sensor_collator_ = absl::make_unique<sensor::TrajectoryCollator>();
  } else {
    LOG(INFO) << "okagv using Collator" ;
...
int MapBuilder::AddTrajectoryBuilder(
  ...
  const int trajectory_id =  current_trajectory_id_;   //trajectory_builders_.size();
  if (options_.use_trajectory_builder_3d()) {
    ...
    trajectory_builders_.emplace(trajectory_id, (absl::make_unique<CollatedTrajectoryBuilder>(
        trajectory_options, sensor_collator_.get(), trajectory_id,
        expected_sensor_ids,
        CreateGlobalTrajectoryBuilder3D(
            std::move(local_trajectory_builder), trajectory_id,
  ...
  *options_with_sensor_ids_proto.mutable_trajectory_builder_options() =
  ...
  all_trajectory_builder_options_.emplace(current_trajectory_id_, options_with_sensor_ids_proto);
...
int MapBuilder::AddTrajectoryForDeserialization(
  ...
  // okagv
    if (pose_graph_->IsTrajectoryExist(current_trajectory_id_)) {
      all_trajectory_builder_options_.at(current_trajectory_id_) =
          options_with_sensor_ids_proto;
    ...
    // all_trajectory_builder_options_.size()); return trajectory_id;
    //const int trajectory_id = trajectory_builders_.size();
    
    trajectory_builders_.emplace(current_trajectory_id_,nullptr);
    all_trajectory_builder_options_.emplace(current_trajectory_id_,
                                            options_with_sensor_ids_proto);
...
std::string MapBuilder::SubmapToProto(
   ...
   {
        LOG(INFO) << "okagv SubmapToProto " << submap_id;
   }
   */
 ...
 //okagv
 /*
 if(submap_id.submap_index == 4 || submap_id.submap_index == 12)
 {
   return "no show";
 }
 */
   ...
   /*
   if (submap_id.submap_index == 4) {

     const SubmapId temp(0,27);

     LOG(INFO) << "okagv Get submap_index 4";
     const auto submap_data = pose_graph_->GetSubmapData(temp);
     if (submap_data.submap == nullptr) {
       return "Requested submap " + std::to_string(temp.submap_index) +
              " from trajectory " + std::to_string(temp.trajectory_id) +
...
std::map<int, int> MapBuilder::LoadState(
  ...
  for (int i = 0; i < pose_graph_proto.trajectory_size(); ++i) {
    ...
    const int new_trajectory_id =
        ...
        //LOG(INFO) << "okagv LoadState new_trajectory_id " << new_trajectory_id; 
    CHECK(trajectory_remapping
              .emplace(trajectory_proto.trajectory_id(), new_trajectory_id)
              .second)
        << "Duplicate trajectory ID: " << trajectory_proto.trajectory_id();
    trajectory_proto.set_trajectory_id(new_trajectory_id);
    if (state == PoseGraphInterface::TrajectoryState::FROZEN) {
      pose_graph_->FreezeTrajectory(new_trajectory_id);
    }
    else
    {
      //okagv
      pose_graph_->SetTrajectoryState(new_trajectory_id,state);
    }
    
  }

  // Apply the calculated remapping to constraints in the pose graph proto.
  //LOG(INFO) << "okagv add pose_graph_proto";
  ...
  //LOG(INFO) << "okagv add submap_poses";
  ...
  //LOG(INFO) << "okagv add landmark_poses";
  ...
  while (deserializer.ReadNextSerializedData(&proto)) {
    switch (proto.data_case()) {
      ...
      case SerializedData::kTrajectoryData: {
        //LOG(INFO) << "okagv add kTrajectoryData";
        //LOG(INFO) << "okagv proto.trajectory_data().trajectory_id() " << proto.trajectory_data().trajectory_id();
      ...
      case SerializedData::kImuData: {
        //LOG(INFO) << "okagv add kImuData";
        if (state == PoseGraphInterface::TrajectoryState::FROZEN) break;
      ...
      case SerializedData::kOdometryData: {
        //LOG(INFO) << "okagv add kOdometryData";
        if (state == PoseGraphInterface::TrajectoryState::FROZEN) break;
      ...
      case SerializedData::kFixedFramePoseData: {
        //LOG(INFO) << "okagv add kFixedFramePoseData";
        if (state == PoseGraphInterface::TrajectoryState::FROZEN) break;
      ...
      case SerializedData::kLandmarkData: {
        //LOG(INFO) << "okagv add kLandmarkData";
        if (state == PoseGraphInterface::TrajectoryState::FROZEN) break;
  ...
  if (state == PoseGraphInterface::TrajectoryState::FROZEN) {
    ...
    //LOG(INFO) << "okagv TrajectoryState FROZEN";
    ...
    // submap.
    //LOG(INFO) << "okagv TrajectoryState not FROZEN";
...
std::map<int, int> MapBuilder::LoadStateFromFile(
  ...
  return LoadState(&stream, state);
}

// okagv
void MapBuilder::SetTrajectoryTypeWithId(TrajectoryType type, int id) {
  ...
  case TrajectoryType::SLAM:
      current_trajectory_id_ = id;
      pose_graph_->SetWorkingTrajectoryType(uint8_t(TrajectoryType::SLAM));
      break;
  case TrajectoryType::NAVIGATION:
      current_trajectory_id_ = 1001;
      pose_graph_->SetWorkingTrajectoryType(uint8_t(TrajectoryType::NAVIGATION));
  case TrajectoryType::IDLE:
      current_trajectory_id_ = id;
      pose_graph_->SetWorkingTrajectoryType(uint8_t(TrajectoryType::IDLE));
  case TrajectoryType::RELOCALIZAION:
      current_trajectory_id_ = id;
      pose_graph_->SetWorkingTrajectoryType(uint8_t(TrajectoryType::RELOCALIZAION));    
  default:
    break;
  }
}

TrajectoryType MapBuilder::GetTrajectoryTypeWithId(int id){
    uint8_t type = pose_graph_->GetWorkingTrajectoryType();
    return TrajectoryType(type);
}

//okagv 
  TrajectoryType MapBuilder::GetWorkingTrajectoryType() {
    uint8_t type = pose_graph_->GetWorkingTrajectoryType();
    return TrajectoryType(type);
  }

  //okagv
  void MapBuilder::DeleteTrajectory(int trajectory_id){
    trajectory_builders_.erase(trajectory_id);
    all_trajectory_builder_options_.erase(trajectory_id);
    sensor_collator_->DeleteTrajectory(trajectory_id);
  }

  // okagv
  bool MapBuilder::SerializeStateToFileWithId(int trajectory_id,
                                              bool include_unfinished_submaps,
                                              const std::string& filename) {
    io::ProtoStreamWriter writer(filename);
    io::WritePbStreamWithId(trajectory_id, *pose_graph_, all_trajectory_builder_options_, &writer,
                      include_unfinished_submaps);
    return (writer.Close());
  }
  ...
  bool MapBuilder::SerializeStateToFileAfterUpdate(
    ...
    io::ProtoStreamWriter writer(filename);
    io::UpdateAndWritePbStream(*pose_graph_, all_trajectory_builder_options_, &writer,
                      include_unfinished_submaps);
    return (writer.Close());
  }

  int MapBuilder::GetTrajectoryIdByName(std::string name) {
  ...
  // okagv
  void MapBuilder::RegisterClientIdForTrajectory(const std::string& client_id,
                                                 int trajectory_id) {
  ...
  //okagv
    void MapBuilder::SetMapBuilderOptions(proto::MapBuilderOptions &option_reset)
    {
      options_ = option_reset;
      pose_graph_->SetPoseGraphOption(const_cast<::cartographer::mapping::proto::PoseGraphOptions&>(
            options_.pose_graph_options()));
    }
