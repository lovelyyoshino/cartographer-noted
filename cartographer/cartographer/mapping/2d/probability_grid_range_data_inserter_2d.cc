...
void CastRaysForIntensity(const sensor::RangeData& range_data,
              const std::vector<uint16>& hit_table,
              const std::vector<uint16>& miss_table,
              const bool insert_free_space, ProbabilityGrid* probability_grid) {
  GrowAsNeeded(range_data, probability_grid);
  ...
  for (const sensor::RangefinderPoint& hit : range_data.returns) {
    ...
    if(hit.intensity > 900)
    {
       probability_grid->ApplyLookupTable(ends.back() / kSubpixelScale, hit_table);
    }
    else
    {
       probability_grid->ApplyLookupTable(ends.back() / kSubpixelScale, miss_table);
    }
...
void ProbabilityGridRangeDataInserter2D::InsertForIntensity(
    const sensor::RangeData& range_data, GridInterface* grid) const {
  ProbabilityGrid* const probability_grid = static_cast<ProbabilityGrid*>(grid);
  CHECK(probability_grid != nullptr);
  // By not finishing the update after hits are inserted, we give hits priority
  // (i.e. no hits will be ignored because of a miss in the same cell).
  CastRaysForIntensity(range_data, hit_table_, miss_table_, options_.insert_free_space(),
           probability_grid);
  probability_grid->FinishUpdate();
};
