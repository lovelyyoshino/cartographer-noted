...
//okagv
Submap2D::Submap2D(const Eigen::Vector2f& origin, std::unique_ptr<Grid2D> grid,
                   std::unique_ptr<Grid2D> intensity_grid,
                   ValueConversionTables* conversion_tables)
    : Submap(transform::Rigid3d::Translation(
          Eigen::Vector3d(origin.x(), origin.y(), 0.))),
      conversion_tables_(conversion_tables) {
  grid_ = std::move(grid);
  intensity_grid_ = std::move(intensity_grid);
}
...
void Submap2D::ToResponseProto(
  ...
  //okagv test
  //intensity_grid()->DrawToSubmapTexture(texture, local_pose());    
...
void Submap2D::InsertRangeData(
  ...
  //okagv
  range_data_inserter->InsertForIntensity(range_data, intensity_grid_.get());
...
void ActiveSubmaps2D::AddSubmap(const Eigen::Vector2f& origin) {
  ...
  */
  ...
  void ActiveSubmaps2D::clearActiveSubmaps()
  {
    submaps_.clear();
  }
