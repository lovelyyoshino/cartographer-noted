...
int MapBuilderBridge::AddTrajectoryWithId(
...
bool MapBuilderBridge::SerializeState(const std::string& filename,
  ...
  return map_builder_->SerializeStateToFileAfterUpdate(
      include_unfinished_submaps, filename);
}

bool MapBuilderBridge::SerializeStateWithId(
    const std::string& filename, int& trajectory_id,
    const bool include_unfinished_submaps) {
...
MapBuilderBridge::GetOKagv_Order() {
  return map_builder_->pose_graph()->GetOKagv_Order();
}

void MapBuilderBridge::SetOKagv_Order(
    ::cartographer::mapping::PoseGraphInterface::OKagvOrder order) {
  map_builder_->pose_graph()->SetOKagv_Order(order);
}

// okagv
::cartographer::mapping::PoseGraphInterface::OKagvFeedback
MapBuilderBridge::GetOKagv_Feedback() {
  return map_builder_->pose_graph()->GetOKagv_Feedback();
}
void MapBuilderBridge::SetOKagv_Feedback(
    ::cartographer::mapping::PoseGraphInterface::OKagvFeedback feedback) {
  map_builder_->pose_graph()->SetOKagv_Feedback(feedback);
}
...
MapBuilderBridge::GetLocalTrajectoryData() {
  ...
  for (const auto& entry : sensor_bridges_) {
    ...
    map_builder_->pose_graph()->GetCovarianceScore(score,is_update);
    ...
    local_trajectory_data[trajectory_id] = {
      ...
      //LOG(INFO) << "Peak.ding time record " << local_slam_data->time;
  }
  ...
  for (const auto& constraint : constraints) {
    ...
    if (constraint.tag ==
      ...
      if (constraint.node_id.trajectory_id ==
        ...
        // Bright yellow
        color_constraint.a = 1.0; //1.0
        color_constraint.r = 0.0; //1.0
        color_constraint.g = 1.0; //1.0
      } else {
        constraint_marker = &constraint_inter_diff_trajectory_marker;
        residual_marker = &residual_inter_diff_trajectory_marker;
        // Bright orange
        color_constraint.a = 1.0;    //1.0
        color_constraint.r = 1.0;    //1.0
        color_constraint.g = 0.0;  //165. / 255.
      }
      // Bright cyan
      color_residual.a = 1.0; //1.0
      color_residual.b = 0.0; //1.0
      color_residual.g = 1.0; //1.0
    }
...
// okagv
void MapBuilderBridge::LoadTrajectory(int trajectory_id,
                                      const std::string& state_filename,
                                      ::cartographer::mapping::PoseGraphInterface::TrajectoryState state) {
  LOG(INFO) << "Loading trajectory with ID '" << trajectory_id << "'...";

  // Make sure there is a trajectory with 'trajectory_id'.
  //CHECK(GetTrajectoryStates().count(trajectory_id));
  map_builder_->SetTrajectoryTypeWithId(TrajectoryType::LOAD,trajectory_id);
  map_builder_->LoadStateFromFile(state_filename, state);
...
void MapBuilderBridge::DeleteTrajectory(int trajectory_id) {
  ...
  // Make sure there is a trajectory with 'trajectory_id'.
  CHECK(GetTrajectoryStates().count(trajectory_id));
  //LOG(INFO) << "Peak.ding map_builder_ DeleteTrajectory";
  map_builder_->DeleteTrajectory(trajectory_id);
  //LOG(INFO) << "Peak.ding pose_graph DeleteTrajectory";
  ...
  if (local_slam_data_.count(trajectory_id) != 0)
  {
      local_slam_data_.erase(trajectory_id);
  }

  //sensor_bridges_.erase(trajectory_id);
}

void MapBuilderBridge::LocalizeTrajectory(
    int trajectory_id, bool use_initial_pose,
    const geometry_msgs::Pose initial_pose) {
  //LOG(INFO) << "Localizing trajectory with ID '" << trajectory_id << "'...";

  // Make sure there is a trajectory with 'trajectory_id'.
  CHECK(GetTrajectoryStates().count(trajectory_id));

  Rigid3d pose = ToRigid3d(initial_pose);
  map_builder_->pose_graph()->LocalizeOKagvPoses(use_initial_pose, trajectory_id, pose);
  //sensor_bridges_.erase(trajectory_id);
}
// okagv
void MapBuilderBridge::SetTrajectoryTypeWithId(TrajectoryType type, int id) {
  map_builder_->SetTrajectoryTypeWithId(type, id);
  if(type == TrajectoryType::SLAM)
  {
     map_builder_->pose_graph()->SetConstraintBuilderMinScore(0.55);
  }
  else if(type == TrajectoryType::NAVIGATION)
  {
    map_builder_->pose_graph()->SetConstraintBuilderMinScore(0.55);
  }
...
// okagv
TrajectoryType MapBuilderBridge::GetTrajectoryTypeWithId(int id) {
  //return map_builder_->GetTrajectoryTypeWithId(id);
   return TrajectoryType::IDLE;
}

// okagv
void MapBuilderBridge::SetTrajectoryStates(
    int trajectory_id,
    ::cartographer::mapping::PoseGraphInterface::TrajectoryState state) {
  map_builder_->pose_graph()->SetTrajectoryState(trajectory_id, state);
}

// okagv
void MapBuilderBridge::GetOkagvOrderStartTrajectoryRequest(
    ::cartographer_ros_msgs::StartTrajectory::Request& request) {
  request.initial_pose = ToGeometryMsgPose(
      map_builder_->okagv_order_start_trajectory_.initial_pose);
  request.relative_to_trajectory_id =
      map_builder_->okagv_order_start_trajectory_.relative_to_trajectory_id;
  request.trajectory_id =
      map_builder_->okagv_order_start_trajectory_.trajectory_id;
  request.trajectory_type =
      map_builder_->okagv_order_start_trajectory_.trajectory_type;
  request.use_initial_pose =
      map_builder_->okagv_order_start_trajectory_.use_initial_pose;
}

void MapBuilderBridge::GetOkagvOrderSaveTrajectoryRequest(
    ::cartographer_ros_msgs::WriteState::Request& request) {
  request.filename = map_builder_->okagv_order_save_trajectory_.filename;
  request.include_unfinished_submaps =
      map_builder_->okagv_order_save_trajectory_.include_unfinished_submaps;
}

void MapBuilderBridge::RegisterClientIdForTrajectory(
    int trajectory_id, std::string trajectory_name) {
  map_builder_->RegisterClientIdForTrajectory(trajectory_name, trajectory_id);
}

ThreadPoolState MapBuilderBridge::GetThreadPoolState() {
  ThreadPoolState state = map_builder_->pose_graph()->GetThreadPoolState();

  switch (state) {
    case ThreadPoolState::ACTIVE:
      LOG(INFO) << "current thread pool is active";
      break;
    case ThreadPoolState::IDLE:
      LOG(INFO) << "current thread pool is idle";
      break;
    default:
      break;
  }
...
// okagv
void MapBuilderBridge::SetMapBuilderOptions(
    cartographer::cloud::proto::MapBuilderServerOptions& option) {
 
  map_builder_->SetMapBuilderOptions(
      const_cast<::cartographer::mapping::proto::MapBuilderOptions&>(
          option.map_builder_options()));
}
