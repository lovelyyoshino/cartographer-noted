...
void SensorBridge::HandleQRCodeLandmarkMessage(
    const std::string& sensor_id,
    const cartographer_ros_msgs::LandmarkList::ConstPtr& msg) {
  auto landmark_data = ToLandmarkData(*msg);
  ...
  if (tracking_from_landmark_sensor != nullptr) {
    for (auto& observation : landmark_data.landmark_observations) {
      observation.landmark_to_tracking_transform =
          *tracking_from_landmark_sensor *
          observation.landmark_to_tracking_transform.inverse();
    }
  }
  trajectory_builder_->AddSensorData(sensor_id, landmark_data);
}

void SensorBridge::HandleReflectorLandmarkMessage(
    const std::string& sensor_id,
  ...
  if (tracking_from_landmark_sensor != nullptr) {
    for (auto& observation : landmark_data.landmark_observations) {
      ...
      observation.landmark_to_map_transform =
          current_tracking_to_map * observation.landmark_to_tracking_transform;
    }
  }
  trajectory_builder_->AddSensorData(sensor_id, landmark_data);
}
...
// okagv
void SensorBridge::HandleReflectorCombinedLandmarkMessage(
    const std::string& sensor_id,
    const cartographer_ros_msgs::LandmarkList::ConstPtr& msg) {
  auto landmark_data = ToLandmarkData(*msg);
...
void SensorBridge::HandleImuMessage(const std::string& sensor_id,
  ...
  imu_data->linear_acceleration[2] = 10.0;
  if (imu_data != nullptr) {
    trajectory_builder_->AddSensorData(
        sensor_id,
...
void SensorBridge::HandleLaserScan(
  ...
  for (int i = 0; i != num_subdivisions_per_laser_scan_; ++i) {
    ...
    //okagv
    std::vector<float> subdivison_intensities(points.intensities.begin() + start_index, points.intensities.end() + end_index);
    HandleRangefinderWithIntensities(sensor_id, subdivision_time, frame_id, subdivision, subdivison_intensities);
  }
}
...
void SensorBridge::HandleRangefinderWithIntensities(
